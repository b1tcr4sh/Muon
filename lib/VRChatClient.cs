...
namespace APITests.lib
{
    public class VRChatClient : PhotonClient
    {
        ...
        private void VRChatClient_JoinedRoom(PhotonClient client)
        {
            ...
            foreach (var Player in CurrentRoom.Players.Values)
            {
                ...
                if (steamID == "0" && bool.Parse(Player.CustomProperties["inVRMode"].ToString()) == false && Player.ActorNumber != client.LocalPlayer.ActorNumber)
                {
                    ...
                    info += $"[CHEATER] {displayname} is using a SteamID Spoofer. Cheater Detected.\n";
                    info += "================================================\n";
                }
                else
                {
        ...
        {
            this.SetCustomProperties(Details.Authentication.AuthParameters.ElementAt(0).Value, Details.Authentication.AuthParameters.ElementAt(1).Value, "avtr_c38a1615-5bf5-42b4-84eb-a8b6c37cbd11"); //example avatar id ok
            this.JoinRoom("wrld_b2d9f284-3a77-4a8a-a58e-f8427f87ba79:92747"); //idk lets find out
        }
    }
}